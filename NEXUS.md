...
| "a simple web application... Spring Boot" | The existing Nexus microservices project (6 services + Angular frontend) | Reused rather than built from scratch. **`product-service`** is the designated flagship service for every grading-relevant demo below — treat it as "the web application" if you're grading against the brief. |
